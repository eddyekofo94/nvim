...
local entry_filter_fuzzy_path, fuzzy_path_option, limit_lsp_types, has_words_before, check_backspace =
  utils_cmp.fuzzy_path_option, utils_cmp.entry_filter_fuzzy_path, utils_cmp.limit_lsp_types, utils_cmp.has_words_before, utils_cmp.check_backspace
    ...
    format = function(entry, vim_item)
      ...
      if vim_item.kind == "File" or vim_item.kind == "Folder" or complpath then
        if string.sub(vim_item.word, #vim_item.word) == "/" then -- Directories
          ...
          if devicons_ok then
            icon, icon_hl = devicons.get_icon(vim.fs.basename(vim_item.word), vim.fn.fnamemodify(vim_item.word, ":e"), { default = true })
            icon = icon and icon .. " "
          end
