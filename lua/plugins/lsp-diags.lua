...
local function setup_keymaps()
  ...
  local function if_supports_lsp_method(method, fn_name, fallback)
    return function()
      return supports_method(method, 0) and string.format('<Cmd>lua vim.lsp.buf["%s"]()<CR>', vim.fn.escape(fn_name, '"\\')) or fallback
    end
  end
  -- stylua: ignore start
  ...
  vim.keymap.set({ 'n', 'x' }, '<Leader>lE', vim.diagnostic.setloclist)
  vim.keymap.set({ 'n', 'x' }, '<Leader><C-e>', vim.diagnostic.setqflist)
  vim.keymap.set({ 'n', 'x' }, '[e', utils.keymaps.count_wrap(vim.diagnostic.goto_prev))
  vim.keymap.set({ 'n', 'x' }, ']e', utils.keymaps.count_wrap(vim.diagnostic.goto_next))
  vim.keymap.set({ 'n', 'x' }, '[E', utils.keymaps.count_wrap(goto_diagnostic('prev', 'ERROR')))
  vim.keymap.set({ 'n', 'x' }, ']E', utils.keymaps.count_wrap(goto_diagnostic('next', 'ERROR')))
  vim.keymap.set({ 'n', 'x' }, '[W', utils.keymaps.count_wrap(goto_diagnostic('prev', 'WARN')))
  vim.keymap.set({ 'n', 'x' }, ']W', utils.keymaps.count_wrap(goto_diagnostic('next', 'WARN')))
  vim.keymap.set({ 'n', 'x' }, '[I', utils.keymaps.count_wrap(goto_diagnostic('prev', 'INFO')))
  vim.keymap.set({ 'n', 'x' }, ']I', utils.keymaps.count_wrap(goto_diagnostic('next', 'INFO')))
  vim.keymap.set({ 'n', 'x' }, '[H', utils.keymaps.count_wrap(goto_diagnostic('prev', 'HINT')))
  vim.keymap.set({ 'n', 'x' }, ']H', utils.keymaps.count_wrap(goto_diagnostic('next', 'HINT')))
  ...
  lsp = {
    ...
    auto_format = {
      ...
      fn_override = function(args, tbl)
        ...
        if args.format then
          fmtopts:scope_action(args, "set", vim.tbl_deep_extend("force", fmtopts:scope_action(args, "get"), args.format))
        end
      end,
    },
    code_action = {
...
local function command_complete(meta, subcommand_info_list)
  ...
  return function(arglead, cmdline, cursorpos)
    ...
    local subcommand = utils.string.camel_to_snake(cmdline:match("^%s*" .. meta .. "(%w+)")) or cmdline:match("^%s*" .. meta .. "%s+(%S+)")
...
local function setup_commands(meta, subcommand_info_list, fn_scope)
  ...
  utils.keymaps.command_abbrev(meta:lower(), meta)
  ...
  for subcommand, _ in pairs(subcommand_info_list) do
    vim.api.nvim_create_user_command(meta .. utils.string.snake_to_camel(subcommand), command_meta(subcommand_info_list, fn_scope, subcommand), {
      bang = true,
      range = true,
      nargs = "*",
      complete = command_complete(meta, subcommand_info_list),
