...
local function term_set_local_keymaps_and_opts(buf)
  if not vim.api.nvim_buf_is_valid(buf) or vim.bo[buf].bt ~= "term" then
    return
  end
...
local function setup(buf)
  ...
  vim.api.nvim_create_autocmd("ModeChanged", {
    ...
    callback = function(info)
      if vim.bo[info.buf].bt == "term" then
  ...
  vim.api.nvim_create_autocmd({ "BufWinEnter", "WinEnter" }, {
    ...
    callback = function(info)
      if vim.bo[info.buf].bt == "term" and vim.b[info.buf].termode == "t" then
        vim.cmd.startinsert()
      end
    end,
  })
