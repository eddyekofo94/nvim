...
function M.map(modes, lhs, rhs, opts)
  ...
  for _, mode in ipairs(mode_list) do
    ...
    if type(existing) == "table" and next(existing) ~= nil then
      ...
      if not (is_same_rhs or is_same_desc) and not warned_keys[conflict_id] then
        conflict_count = conflict_count + 1

        local path = existing.sid > 0 and vim.fn.expand("<script:" .. existing.sid .. ">") or "Internal/Built-in"
        local plugin_name = path:match "lazy/([^/]+)" or "User Config"
        local action = (existing.rhs and existing.rhs ~= "") and existing.rhs or "Lua function"
        ...
        vim.notify(string.format("Conflict #%d: [%s]", conflict_count, lhs), vim.log.levels.WARN)
      end
    end
  end
...
function M.get_conflicts()
  ...
  local report = { "📊 KEYMAP CONFLICT AUDIT", "Total unique collisions: " .. conflict_count, "" }
  ...
  for _, data in pairs(warned_keys) do
    ...
    table.insert(report, string.format("   📍 Source: %s:%s", data.path, data.line))
...
function M.fallback_fn(key_def)
  return function()
    ...
    elseif key_def.rhs then
      ...
      local keys = vim.api.nvim_replace_termcodes(key_def.rhs, true, true, true)
...
function M.command_abbrev(trig, command, opts)
  ...
  local expansion_fn = function()
    ...
    if cmd_type == ":" and vim.fn.getcmdcompltype() == "command" and is_start then
      return command
    end
    return trig
  end
...
function M.command_map(trig, command, opts)
  local expansion_fn = function()
    ...
    if cmd_type == ":" and cmd_line == "" and vim.fn.getcmdcompltype() == "command" then
      return command
    end
    return trig
  end
...
function M.close_floats(k)
  ...
  local floats = vim.iter(vim.api.nvim_tabpage_list_wins(0)):filter(function(win)
    return vim.fn.win_gettype(win) == "popup"
      ...
      and not vim.tbl_contains({ "cmd", "dialog", "msg", "pager" }, vim.bo[vim.fn.winbufnr(win)].ft)
  end)
